...
const makeList = async(catalogs: Catalog[], path, d: Document) => {
  ...
  for (const unit of catalogs) {
    ...
    const routerLink: string = (unit.showPath + unit.name).replace(path, '')
...
const initSubList = async (config: Config) => {
  ...
  const subListArr: Element[] = (<Array<Element>>Array.from(subList))
  ...
  const handle: Function = (container: Element, first?: boolean): void => {
    ...
    const list: HTMLElement = (<HTMLElement>container.querySelector('.sub-list'))
...
const changTitle = (eventHub: any, config: Config) => {
  const pathToTitle = (p: string): string => {
  ...
  eventHub.listen('container-changed', ({ detail }) => {
    document.title = `${decodeURI(pathToTitle(detail))} - ${config.title}`
  })
}
